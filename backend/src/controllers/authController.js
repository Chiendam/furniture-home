...
exports.protect = catchAsync( async (req, res, next) => {
  // 1. get token and check of it's thereof
  let token;
  ...
  if(!token){
    return next(new customError('You not logged in! Please log in to get access. ', 401))
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const message = `Đây là email reset lại mật khẩu. Vui lòng click vào đường dẫn ${urlReset} để tiếp tục reset mật khẩu.\nNếu không phải bạn yêu cầu reset mật khẩu xin vui lòng bỏ qua email!`

  try {
    await sendEmail({
      email: user.email,
      subject: 'Reset mật khẩu - đường dẫn reset mật Khẩu có hiện lực trong 10 phút',
      message
    });
